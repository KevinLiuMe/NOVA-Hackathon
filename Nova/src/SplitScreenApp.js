import { Send, Copy, Trash2, TrendingUp, TrendingDown, Activity, DollarSign, BarChart2, Award, AlertTriangle, Wrench } from 'lucide-react';
...
const SplitScreenApp = () => {
  const [messages, setMessages] = useState([
    { type: 'ai', text: "Hello! I'm Nova, your AI trading assistant. How can I help you today?" }
  ]);
  ...
  const handleFixCode = async () => {
    ...
    setMessages(prevMessages => [
      ...prevMessages,
      { type: 'ai', text: "I'll try to fix the code. Analyzing the error..." }
    ]);
    ...
    try {
      ...
      if (data.isStrategy) {
        ...
        setMessages(prevMessages => [
          ...prevMessages,
          { type: 'ai', text: "I've fixed the strategy code. You can review the changes in the editor and try running it again." }
        ]);
      } else {
        setMessages(prevMessages => [
          ...prevMessages,
          { type: 'ai', text: "I apologize, but I couldn't generate a fix for the code. Please check the error message and try modifying the code manually." }
        ]);
      }
    } catch (error) {
      ...
      setMessages(prevMessages => [
        ...prevMessages,
        { type: 'ai', text: "Sorry, I encountered an error while trying to fix the code. Please try again." }
        ]);
    }
  };
